 ...
 */
 export function fillTemplateString(templateString, placeholderValuesObject) {
 ...
 */
export function isMemberExpressionIdentifier(identifier) {
  return /^[\d\w]+(\.[\d\w]+)+$/u.test(identifier);
}
 ...
 */
export function isValidModuleSource(sourceValue) {
 ...
 */
export function isWhitelistedModuleSource(sourceValue, whitelistedModuleSourceList) {
  if (isRelativePathSource(sourceValue)) return true;

  let isValidImport = false;

  whitelistedModuleSourceList.forEach(function (whitelistedModule) {
    const regexString = `^${whitelistedModule}(/[\\d\\w]+)*$`;
